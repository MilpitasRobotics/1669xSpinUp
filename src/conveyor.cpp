...
void move_conveyor(){ //driver control
  ...
  while (master.get_digital(DIGITAL_L2) && conveyor_lock) { // this elif statement will execute after L1 is held down since L1 will no longer be pressed and conveyer_lock == true
    conveyorRoller.move_velocity(600);
  }

  if (conveyor_up) // if the catapult is not in loading position, the intake will not run to prevent jamming
    conveyorRoller.move_velocity(-600);
  else
