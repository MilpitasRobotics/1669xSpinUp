...
void move_catapult(double speed){
  ...
  if (!catapult_switch.get_value() && isEnabled && anotherOne){
    catapultMotor.move_velocity(speed);
    canFire = false;
  }else if (stop && catapult_switch.get_value() && counter == 1){ // executes and tells catapult to stop at bottom
    catapultMotor.move_velocity(0);  
    fireCatapult = false; 
